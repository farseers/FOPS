  ...
  <div class="system-user-container layout-padding">
    <el-card shadow="hover" class="layout-padding-auto">
      <div class="system-user-search mb15">
        ...
        <label class="ml10">事件</label>
        <el-input class="ml5" size="default" v-model="state.queueName" placeholder="事件名称" clearable
...
const getTableData = () => {
  ...
  // 请求接口
  serverApi.linkTraceEventList(params).then(function (res) {
    if (res.Status) {
...
const linkTraceDelete = () => {
  ElMessageBox.confirm(`删除七天前的数据，是否继续?`, '提示', {
    ...
    .then(() => {
      serverApi.linkTraceDelete({ traceType: 6 }).then((res) => {
        if (res.Status) {
          onQuery();
