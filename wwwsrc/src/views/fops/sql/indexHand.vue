<template>
	<div class="system-user-container layout-padding">
        ...
        <el-input class="ml5" size="default" v-model="state.name" placeholder="名称" clearable style="max-width: 180px;"> </el-input>
...
const linkTraceDeleteSlow = ()=>{
  ElMessageBox.confirm(`删除七天前的数据，是否继续?`, '提示', {
		...
		.then(() => {
			serverApi.linkTraceDelete({dbName:'Hand'}).then((res)=>{
				if(res.Status){
					onQuery();
					ElMessage.success('删除成功');
				}else{
