...
type chContext struct {
	...
	TraceDetail      data.TableSet[model.TraceDetailPO]      `data:"name=trace_detail;migrate=ReplacingMergeTree() ORDER BY (app_name,parent_app_name,app_ip,trace_id,start_ts,detail_id) PARTITION BY (call_type)"`
