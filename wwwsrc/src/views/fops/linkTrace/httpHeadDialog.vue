          ...
          <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
            <ul class="custom-list">
              <li>
                ...
                <el-tag size="small">{{ state.ruleForm.HttpStatusCode }}</el-tag> {{ state.ruleForm.RequestIp }} <el-tag
                  type="success" size="small">{{ state.ruleForm.HttpMethod }}</el-tag>
                <el-tag v-if="state.ruleForm.HttpContentType != null" type="info"
                  size="small">{{ state.ruleForm.HttpContentType }}</el-tag>
                <el-tag style="font-size: 14px;"
                  @click="copyToClipboard(state.ruleForm.HttpUrl)">{{ state.ruleForm.HttpUrl }}</el-tag>
                耗时：
                <el-tag size="small" v-if="state.ruleForm.UseTs > 100000000"
                  type="danger">{{ state.ruleForm.UseDesc }}</el-tag>
                <el-tag size="small" v-else-if="state.ruleForm.UseTs > 50000000"
                  type="warning">{{ state.ruleForm.UseDesc }}</el-tag>
              ...
              <div style="display: flex; gap: 10px; list-style: none; padding: 0;">
                <li style="flex: 1;"><el-tag size="small">请求Headers：</el-tag>
                  <el-button size="small"
                    @click="copyToClipboard(formatJson(friendlyJSONstringify(state.ruleForm.HttpHeaders)))"
                    type="info">复制</el-button>
                  <pre>{{ formatJson(friendlyJSONstringify(state.ruleForm.HttpHeaders)) }}</pre>
                </li>
                <li style="flex: 1;"><el-tag size="small">响应Headers：</el-tag>
                  <el-button size="small"
                    @click="copyToClipboard(formatJson(friendlyJSONstringify(state.ruleForm.HttpResponseHeaders)))"
                    type="info">复制</el-button>
              ...
              <div style="display: flex; gap: 10px; list-style: none; padding: 0;">
                <li style="flex: 1;"><el-tag size="small">入参：</el-tag>
                  <el-button size="small" @click="copyToClipboard(formatJson(state.ruleForm.HttpRequestBody))"
                    type="info">复制</el-button>
                  <pre>{{ formatJson(state.ruleForm.HttpRequestBody) }}</pre>
                </li>
                <li style="flex: 1;"><el-tag size="small">出参：</el-tag>
                  <el-button size="small" @click="copyToClipboard(formatJson(state.ruleForm.HttpResponseBody))"
                    type="info">复制</el-button>
...
const openDialog = (type: number, row: any) => {
  if (type == 2) {
    ...
    row.HttpResponseHeaders = row.WebResponseHeaders
