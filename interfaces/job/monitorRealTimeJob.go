			...
			if !rule.IsNull() && send {
				...
				case ">":
					if parse.ToFloat32(rule.KeyValue) > parse.ToFloat32(reqVal) {
						comparisonMsg = fmt.Sprintf("%s %s %s 大于 %f", time.Now().Format("01-02 15:04:05"), rule.AppName, rule.AppName, parse.ToFloat32(reqVal))
					}
				case "<":
					if parse.ToFloat32(rule.KeyValue) < parse.ToFloat32(reqVal) {
						comparisonMsg = fmt.Sprintf("%s %s %s 小于 %f", time.Now().Format("01-02 15:04:05"), rule.AppName, rule.AppName, parse.ToFloat32(reqVal))
					}
				case "=":
					if parse.ToFloat32(rule.KeyValue) == parse.ToFloat32(reqVal) {
						comparisonMsg = fmt.Sprintf("%s %s %s 等于 %f", time.Now().Format("01-02 15:04:05"), rule.AppName, rule.AppName, parse.ToFloat32(reqVal))
