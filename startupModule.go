...
func (module StartupModule) PostInitialize() {
	...
	if proxyAgent := configure.GetString("Fops.Proxy"); proxyAgent != "" {
		...
		exec.RunShellCommand("git", []string{"config", "--global", "http.https://github.com.proxy", proxyAgent}, nil, "", true)
		exec.RunShellCommand("git", []string{"config", "--global", "https.https://github.com.proxy", proxyAgent}, nil, "", true)
	} else {
		flog.Info("未开启Git代理")
		exec.RunShellCommand("git", []string{"config", "--global", "--unset", "http.https://github.com.proxy"}, nil, "", false)
		exec.RunShellCommand("git", []string{"config", "--global", "--unset", "https.https://github.com.proxy"}, nil, "", false)
	}
	// 初始化目录
	apps.InitFopsDir()
}
