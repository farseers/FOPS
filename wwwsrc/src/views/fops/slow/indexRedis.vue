  ...
  <div class="system-user-container layout-padding">
    <el-card shadow="hover" class="layout-padding-auto">
      <div class="system-user-search mb15">
        ...
        <el-input class="ml5" size="default" v-model="state.methodName" placeholder="Method" clearable
          style="max-width: 160px;"> </el-input>
        <label class="ml10">KEY</label>
        <el-input class="ml5" size="default" v-model="state.key" placeholder="Key" clearable style="max-width: 160px;">
        </el-input>
        <label class="ml10">Field</label>
        <el-input class="ml5" size="default" v-model="state.field" placeholder="Field" clearable
          style="max-width: 160px;"> </el-input>
      ...
      <el-table :data="state.tableData.data" v-loading="state.tableData.loading" style="width: 100%">
        ...
        <el-table-column width="160px" prop="MethodName" label="MethodName" show-overflow-tooltip></el-table-column>
        <el-table-column prop="RedisKey" label="Key" show-overflow-tooltip>
          <template #default="scope">
            <el-tag size="small" type="info">{{ scope.row.RedisKey }}</el-tag><br>
            {{ scope.row.RedisField }}
          </template>
        </el-table-column>
        <el-table-column width="80px" prop="RedisRowsAffected" label="行数" show-overflow-tooltip></el-table-column>
        <el-table-column width="200px" label="异常" show-overflow-tooltip>
          <template #default="scope">
            <el-tag size="small" v-if="scope.row.Exception != null"
              type="danger">{{ scope.row.Exception.ExceptionCallFile }}:{{ scope.row.Exception.ExceptionCallLine }}
...
const getTableData = () => {
  ...
  // 请求接口
  serverApi.slowRedis(params).then(function (res) {
    if (res.Status) {
...
const linkTraceDeleteSlow = () => {
  ElMessageBox.confirm(`删除七天前的数据，是否继续?`, '提示', {
    ...
    .then(() => {
      serverApi.linkTraceDeleteSlow({ dbName: 'Redis' }).then((res) => {
        if (res.Status) {
          onQuery();
          ElMessage.success('删除成功');
        } else {
