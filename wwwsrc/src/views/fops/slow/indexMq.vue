  ...
  <div class="system-user-container layout-padding">
    <el-card shadow="hover" class="layout-padding-auto">
      <div class="system-user-search mb15">
        ...
        <label class="ml10">服务地址</label>
        <el-input class="ml5" size="default" v-model="state.server" placeholder="服务地址" clearable
          style="max-width: 180px;"> </el-input>
        <label class="ml10">交换器</label>
        <el-input class="ml5" size="default" v-model="state.exchange" placeholder="交换器" clearable
          style="max-width: 180px;"> </el-input>
        <label class="ml10">路由KEY</label>
        <el-input class="ml5" size="default" v-model="state.routingKey" placeholder="路由KEY" clearable
          style="max-width: 180px;"> </el-input>
      ...
      <el-table :data="state.tableData.data" v-loading="state.tableData.loading" style="width: 100%">
        ...
        <el-table-column prop="MqServer" label="服务器地址" show-overflow-tooltip></el-table-column>
        <el-table-column prop="MqExchange" label="交换器" show-overflow-tooltip></el-table-column>
        <el-table-column prop="MqRoutingKey" label="路由KEY" show-overflow-tooltip></el-table-column>
...
const getTableData = () => {
  ...
  // 请求接口
  serverApi.slowMq(params).then(function (res) {
    if (res.Status) {
...
const linkTraceDeleteSlow = () => {
  ElMessageBox.confirm(`删除七天前的数据，是否继续?`, '提示', {
    ...
    .then(() => {
      serverApi.linkTraceDeleteSlow({ dbName: 'Mq' }).then((res) => {
        if (res.Status) {
          onQuery();
          ElMessage.success('删除成功');
        } else {
