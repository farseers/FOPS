<template>
	<div class="system-user-container layout-padding">
        ...
        <label class="ml10">索引名称</label>
        <el-input class="ml5" size="default" v-model="state.indexName" placeholder="索引名称" clearable style="max-width: 180px;"> </el-input>
        <label class="ml10">别名名称</label>
        <el-input class="ml5" size="default" v-model="state.aliasesName" placeholder="别名名称" clearable style="max-width: 180px;"> </el-input>
...
const linkTraceDeleteSlow = ()=>{
  ElMessageBox.confirm(`删除七天前的数据，是否继续?`, '提示', {
		...
		.then(() => {
			serverApi.linkTraceDeleteSlow({dbName:'Es'}).then((res)=>{
				if(res.Status){
					onQuery();
