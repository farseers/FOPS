            ...
            <div class="mt10">
              <span v-if="state.TraceType == 0">
                ...
                <el-tag v-if="state.WebContentType != ''" type="info" size="small">{{ state.WebContentType }}</el-tag>{{
                  state.WebPath }}
                <el-button style="margin-left: 20px" type="primary" @click="onShowHeadDialog()"
                  size="small">查看报文</el-button>
              </span>
              ...
              <div v-if="state.Exception != null" class="mt5">
                <el-tag type="danger">
                  <div v-if="state.Exception.ExceptionDetails != null">
                    异常：{{ state.Exception.ExceptionDetails[0].ExceptionCallFile }}:{{
                      state.Exception.ExceptionDetails[0].ExceptionCallLine }}
                    {{ state.Exception.ExceptionDetails[0].ExceptionCallFuncName }}</div>
                      ...
                      <div class="el-progress el-progress--line is-exception el-progress--text-inside"
                        ...
                        <div class="el-progress-bar">
                          <div class="el-progress-bar__inner"
                            :style="{ 'height': '21px', 'width': '100%', 'animation-duration': '3s', 'text-align': 'left', 'background-color': 'rgb(' + info.Rgba + ')' }">
                            <div class="el-progress-bar__innerText" style="color:#181818">
                              <el-tag size="small" style="margin-right: 5px;">{{ info.AppName }}</el-tag>
                              <el-tag size="small" style="margin-right: 5px;" v-if="info.Exception != null"
                                :title="info.Exception.ExceptionMessage" type="danger">【异常】</el-tag>
                              <span v-html="info.Caption"></span>
                              <span v-if="index > 0 && info.UseTs > 0">，耗时：
                                <el-tag size="small" v-if="info.UseTs > 100000000" type="danger">{{ info.UseDesc
                                }}</el-tag>
                                <el-tag size="small" v-else-if="info.UseTs > 50000000" type="warning">{{ info.UseDesc
                                }}</el-tag>
...
const showDialog = defineAsyncComponent(() => import('/src/views/fops/linkTrace/httpHeadDialog.vue'));
...
const openDialog = (row2: any) => {
  ...
  serverApi.linkTraceInfo(row2.TraceId).then(function (res) {
    ...
    if (res.Status) {
      ...
      state.WebResponseHeaders = res.Data.Entry.wrsph
...
const onShowHeadDialog = () => {
  showDialogRef.value.openDialog(2, state);
}
const showLog = () => {
  logDetailDialogRef.value.openDialog(state.traceInfo);
}
