<template>
	<div class="system-user-container layout-padding">
        ...
        <el-table-column label="请求内容" show-overflow-tooltip>
          <template #default="scope">
            {{scope.row.WebRequestIp}} <el-tag type="success" size="small">WS</el-tag>
...
const getTableData = () => {
  ...
  // 请求接口
  serverApi.linkTraceWebSocket(params).then(function (res){
    if (res.Status){
...
const linkTraceDelete = ()=>{
  ElMessageBox.confirm(`删除七天前的数据，是否继续?`, '提示', {
		...
		.then(() => {
			serverApi.linkTraceDelete({traceType:7}).then((res)=>{
				if(res.Status){
					onQuery();
