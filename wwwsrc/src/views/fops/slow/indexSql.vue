  ...
  <div class="system-user-container layout-padding">
    <el-card shadow="hover" class="layout-padding-auto">
      <div class="system-user-search mb15">
        ...
        <el-input class="ml5" size="default" v-model="state.dbName" placeholder="数据库名称" clearable
          style="max-width: 180px;"> </el-input>
        <label class="ml10">表名称</label>
        <el-input class="ml5" size="default" v-model="state.tableName" placeholder="表名称" clearable
          style="max-width: 180px;"> </el-input>
      ...
      <el-table :data="state.tableData.data" v-loading="state.tableData.loading" style="width: 100%">
        ...
        <el-table-column label="Sql语句" show-overflow-tooltip>
          <template #default="scope">
            <el-tag size="small" v-if="scope.row.DbName != ''">{{ scope.row.DbName }}</el-tag> {{ scope.row.DbTableName }}
            <div style="cursor: pointer" title="点击复制" @click="copyText(scope.row.DbSql)">{{ scope.row.DbSql }}</div>
          </template>
        </el-table-column>
...
const linkTraceDeleteSlow = () => {
  ElMessageBox.confirm(`删除七天前的数据，是否继续?`, '提示', {
    ...
    .then(() => {
      serverApi.linkTraceDeleteSlow({ dbName: 'Db' }).then((res) => {
        if (res.Status) {
          onQuery();
          ElMessage.success('删除成功');
        } else {
